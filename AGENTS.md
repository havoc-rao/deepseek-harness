...
- **Pull requests are opt-in.** Creating, updating, or merging PRs — `gh pr` calls, PR bodies, base/head targets, labels, and merge actions — happens only on explicit instruction; otherwise work stops at committed, pushed branches.
