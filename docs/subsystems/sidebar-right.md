...
One docking surface exists per Session, plus one session-independent surface while no Session is current; both are drawn by the session-maybe seat `rightbar.session`, which keeps the session-independent surface under a reserved key ([state](../../packages/client/ui-sidebar-right/README.md#state)). The root-scoped `rightbar` controller mounts that seat only while Conversation is selected; a reload returns every surface to the collapsed default, and switching sessions keeps each one where it was. The surface's every change is one recorded history entry computed by the kit's pure planners; a docked pane never stays empty, and an empty root pane receives the default page selected from registered guide entries.
