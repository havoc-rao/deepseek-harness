...
- `recentOutputs` is the distinct-path ledger of those same write/edit mutations in recency order — most recently modified first, a re-modification moving the path back to the front — trimmed to the newest 32 paths on the wire. `filesChanged` keeps counting the full ledger, so older paths leave `recentOutputs` without changing the totals.
- `recentInputs` is the parallel ledger of the session's read files — the input sources: every successful read result whose `tool/result` `meta` carries the read tool's structured window (`path` + `offset` + `lines`), in recency order, trimmed to the newest 32. The workspace browser's session hover card renders both ledgers as directory trees under the input/output headings; `filesChanged` and the line totals remain output-only.
...
- **Change totals are the tools' applied diffs, not a git stat** — only mutations that record applied diffs in `tool/result` `meta` (write/edit) count, and the lines follow the diff-card convention: context lines around each hunk count on both sides, and repeated edits of one file accumulate instead of netting out. The figures answer "how much the session's file tools changed", not `git diff --stat`. The input ledger is likewise meta-scoped: only reads that persist the read tool's structured window count — `str_replace_editor`'s `view`, `read_image`, and search reads record no such window and contribute nothing.
- **The path ledgers grow with the log** — every distinct read and changed path is retained in the fold state (the exact-distinct-count oracles), so a session touching many files grows its persisted projection row; the wire lists are trimmed to the newest 32, but the state itself is not.
