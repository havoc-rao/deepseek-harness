...
- `recentOutputs` 是这些 write/edit 变更的同一不同路径台账的按最近修改倒序视图——最新修改在前，再次修改会把该路径移回最前——线上只保留最新 32 个路径。`filesChanged` 仍统计完整台账，因此旧路径离开 `recentOutputs` 不会改变累计数字。
- `recentInputs` 是会话读取文件的平行台账——输入源：每个成功的读取结果，其 `tool/result` `meta` 携带 read 工具的结构化窗口（`path` + `offset` + `lines`），按最近读取倒序，线上保留最新 32 个。工作区浏览器的会话悬停卡片把两个台账渲染为输入/输出两个标题下的目录树；`filesChanged` 与增删行数仍只统计输出。
...
- **路径台账随日志增长**——每个不同的已读与已改路径都保留在折叠状态中（精确去重计数的依据），因此涉及大量文件的会话会让其持久化的 projection 行变大；线上列表裁剪到最新 32 个，但状态本身不裁剪。输入台账同样按 meta 口径：只有持久化 read 工具结构化窗口的读取计入——`str_replace_editor` 的 `view`、`read_image` 与搜索读取没有该窗口，不计入。
