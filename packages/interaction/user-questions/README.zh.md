...
- `ctx.on('userQuestions/ask', (request) => …)`：在验证通过的问题送达当前提供方的那一刻触发——即问题接缝的阻塞边界。只观测、非否决：监听器不能改变问题或其回答，无论有没有观测者，提问都会继续。抛异常的监听器会被隔离并记录日志，绝不会使提问失败。`dsh-hooks-claude-code` 桥接会从该事件触发其 `Notification`（`elicitation_dialog`）hook 点。
