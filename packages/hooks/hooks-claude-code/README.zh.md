...
| `Notification` | `approval/request`（瀑布观察者）＋ `userQuestions/ask`（emit 观察者） | detached 的用户关注触发：审批问题触发 `permission_prompt`，用户提问触发 `elicitation_dialog`；只观测，不能改变结果 |
| `PermissionRequest` | `approval/request`（瀑布观察者） | detached 的权限触发（审批问题）：携带 `permission_mode: "ask"` 及 `tool_name`／`tool_use_id`／`reason`；只观测，不能改变结果 |

五个 emit 点（SessionStart／SubagentStart／SubagentStop／Notification／PermissionRequest）都以分离方式运行：没有扩展点会等待这些 hook。每条运行链都会被跟踪；对桥接执行 dispose（资源释放）时，会中止仍在运行的 hook 进程，并在 dispose 完成前排空 continuation（`createDetachedRuns`，位于 `dsh-hook-protocol`）。`Notification` 或 `PermissionRequest` hook 绝不延迟或使其宣告的审批/问题失败：两者都在任何应答监听器之前于审批瀑布上触发（prepend），但总是用 `next()` 委托，并以 detached、受控方式运行（runHook 从不 reject，因此触发不可能抛入瀑布）。

matcher subject 是工具名称（`PreToolUse`／`PostToolUse`／`PermissionRequest`）、会话源（`SessionStart`）、`notification_type`（`Notification`），或常量 `agent_type`，其值为 `general-purpose`（`SubagentStart`／`SubagentStop`）。harness subagent seam 不携带每 kind label，因此桥接报告 Claude Code 自身 Task 工具默认值；默认／`*`／空 `agent_type` matcher 会触发，特定 kind matcher 不会触发。`UserPromptSubmit`／`Stop` 忽略 matcher。一个点上文件配置的多个 hook 会**按配置顺序串行运行**，并按最严格方式折叠（`deny > ask > allow`，见 `dsh-hook-protocol`）。串行使每个 hook 的 `hook/invoked`／`hook/result` 对在日志中相邻，权限决策的折叠结果与顺序无关（见 Agent Note 的「run serially, not concurrently」说明）。
...
- **不支持的 hook 事件（Claude Code 当前 30 项中的 21 项）：** `Setup`、`InstructionsLoaded`、`UserPromptExpansion`、`MessageDisplay`、`PostToolUseFailure`、`PostToolBatch`、`PermissionDenied`、`TaskCreated`、`TaskCompleted`、`StopFailure`、`TeammateIdle`、`ConfigChange`、`CwdChanged`、`FileChanged`、`WorktreeCreate`、`WorktreeRemove`、`PreCompact`、`PostCompact`、`SessionEnd`、`Elicitation` 和 `ElicitationResult`。这些事件的配置会在配置组解析前被忽略，因此不支持的事件既不会使配置失效，也不会注册 hook。比较基线是 Claude Code [官方 hook 事件参考](https://code.claude.com/docs/en/hooks#hook-events)。
- **`Notification` 只支持部分功能：** 桥接只对 harness 暴露的两个人类阻塞等待触发——审批（`notification_type: "permission_prompt"`）与用户提问（`notification_type: "elicitation_dialog"`），只带 `message`，权限触发时另带 `tool_name`／`tool_use_id`／`reason`。Claude Code 的其他通知类型（`idle_prompt`、`auth_success`）与字段不会生成，且该 hook 不能改变它宣告的审批/问题。
- **`PermissionRequest` 只支持部分功能：** 桥接只对 harness 的 `ask` 审批策略触发（即瀑布确实派发的场景），携带 `permission_mode: "ask"` 及 `tool_name`／`tool_use_id`／`reason`。它不能自行批准或拒绝该请求——决定权仍在组合起来的应答方——且 Claude Code 在其它权限模式下报告的那些字段不会生成。
...
- **通用 payload 与输出字段只支持部分功能：** 已映射事件会省略 Claude Code 原本会提供的 `prompt_id`、`transcript_path` 和 `effort`。`PermissionRequest` 是唯一携带 `permission_mode`（`"ask"`）的点。`systemMessage` 会被记录 + 警告但不呈现；`{"continue": false}` 会被记录但不会停止运行；不会应用 `suppressOutput`、`stopReason` 和 `terminalSequence`（`TODO(hook-continue-false)`）。
