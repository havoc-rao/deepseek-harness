...
The four emit points (SessionStart/SubagentStart/SubagentStop/Notification) run detached — no extension point awaits these hooks. Each run chain is tracked, and disposing the bridge aborts still-running hook processes, then drains the continuations before the dispose resolves (`createDetachedRuns` in `dsh-hook-protocol`). A `Notification` hook never delays or fails the approval/question it announces: it fires on the approval waterfall before any answering listener (prepend) but always delegates with `next()`, and it runs detached and contained (runHook never rejects, so the fire cannot throw into the waterfall).

The matcher subject is the tool name (`PreToolUse`/`PostToolUse`), the session source (`SessionStart`), the `notification_type` (`Notification`), or a constant `agent_type` of `general-purpose` (`SubagentStart`/`SubagentStop` — the harness subagent seam carries no per-kind label, so the bridge reports Claude Code's own Task-tool default; a default/`*`/empty `agent_type` matcher fires, a specific-kind matcher does not); `UserPromptSubmit`/`Stop` ignore matchers. Multiple file-configured hooks on one point run **serially, in config order**, and fold most-restrictively (`deny > ask > allow`, see `dsh-hook-protocol`); serial keeps each hook's `hook/invoked`/`hook/result` pair adjacent in the log, and the fold is order-independent for the decision (see the Agent Note's "run serially, not concurrently" note).
