...
| `Notification` | `approval/request` (waterfall observer) + `userQuestions/ask` (emit observer) | detached user-attention fire: `permission_prompt` on an approval question, `elicitation_dialog` on a user question; observe-only, cannot change the outcome |
| `PermissionRequest` | `approval/request` (waterfall observer) | detached permission fire on an approval question: carries `permission_mode: "ask"` plus `tool_name`/`tool_use_id`/`reason`; observe-only, cannot change the outcome |

The five emit points (SessionStart/SubagentStart/SubagentStop/Notification/PermissionRequest) run detached — no extension point awaits these hooks. Each run chain is tracked, and disposing the bridge aborts still-running hook processes, then drains the continuations before the dispose resolves (`createDetachedRuns` in `dsh-hook-protocol`). A `Notification` or `PermissionRequest` hook never delays or fails the approval/question it announces: both fire on the approval waterfall before any answering listener (prepend) but always delegate with `next()`, and run detached and contained (runHook never rejects, so the fire cannot throw into the waterfall).

The matcher subject is the tool name (`PreToolUse`/`PostToolUse`/`PermissionRequest`), the session source (`SessionStart`), the `notification_type` (`Notification`), or a constant `agent_type` of `general-purpose` (`SubagentStart`/`SubagentStop` — the harness subagent seam carries no per-kind label, so the bridge reports Claude Code's own Task-tool default; a default/`*`/empty `agent_type` matcher fires, a specific-kind matcher does not); `UserPromptSubmit`/`Stop` ignore matchers. Multiple file-configured hooks on one point run **serially, in config order**, and fold most-restrictively (`deny > ask > allow`, see `dsh-hook-protocol`); serial keeps each hook's `hook/invoked`/`hook/result` pair adjacent in the log, and the fold is order-independent for the decision (see the Agent Note's "run serially, not concurrently" note).
...
- **Unsupported hook events (21 of Claude Code's current 30):** `Setup`, `InstructionsLoaded`, `UserPromptExpansion`, `MessageDisplay`, `PostToolUseFailure`, `PostToolBatch`, `PermissionDenied`, `TaskCreated`, `TaskCompleted`, `StopFailure`, `TeammateIdle`, `ConfigChange`, `CwdChanged`, `FileChanged`, `WorktreeCreate`, `WorktreeRemove`, `PreCompact`, `PostCompact`, `SessionEnd`, `Elicitation`, and `ElicitationResult`. Config for these events is ignored before group parsing, so an unsupported event cannot invalidate or register hooks. The comparison baseline is Claude Code's [official hook-event reference](https://code.claude.com/docs/en/hooks#hook-events).
- **`Notification` is partial:** the bridge fires it only for the two human-blocking waits the harness exposes — approvals (`notification_type: "permission_prompt"`) and user questions (`notification_type: "elicitation_dialog"`), with only `message`, plus `tool_name`/`tool_use_id`/`reason` on the permission fire. Other Claude Code notification types (`idle_prompt`, `auth_success`) and fields are not produced, and the hook cannot change the approval/question it announces.
- **`PermissionRequest` is partial:** the bridge fires it only for the harness's `ask` approval policy (the case where the waterfall dispatches at all), carrying `permission_mode: "ask"` plus `tool_name`/`tool_use_id`/`reason`. It cannot itself approve or deny the request — the decision stays with the composed answerers — and fields Claude Code reports on other permission modes are not produced.
...
- **Common payload and output fields are partial:** mapped event payloads omit `prompt_id`, `transcript_path`, and `effort` where Claude Code would provide them. `PermissionRequest` is the one point that carries `permission_mode` (`"ask"`). `systemMessage` is logged + warned but not surfaced; `{"continue": false}` is recorded but does not halt the run; `suppressOutput`, `stopReason`, and `terminalSequence` are not applied (`TODO(hook-continue-false)`).
