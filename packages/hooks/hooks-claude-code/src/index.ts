...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.on('approval/request', (req: ApprovalRequest, next) => {
    ...
    fire('PermissionRequest', req.toolName, {
      ...base(req.agent, 'PermissionRequest'),
      permission_mode: 'ask',
      ...approvalFields,
    }, req.agent)
    notify(
  ...
  // The elicitation observer fires BEFORE any answering listener (prepend) so
  // the notification lands, but never consumes the waterfall — it delegates
  // with `next()` (scope-filtered dispatch: agent-scoped listeners receive
  // only that agent, a root-level observer sees every ask).
