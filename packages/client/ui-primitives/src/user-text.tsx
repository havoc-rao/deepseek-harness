...
/**
 * One decorated reference token's display projection, handed to a consumer
 * that replaces the inline chip; identity remains in the original message.
 */
export interface UserTextReferenceOwner {
  token: string
  label: string
  displayLabel: string
  kind: 'file' | 'folder' | 'session' | 'skill' | 'command'
}
 ...
 * @param renderReference - optional replacement renderer for one decorated token; returning
 *   undefined keeps the shipped chip (an extension declines tokens it does not own).
...
export function projectUserText(
  ...
  renderReference?: (owner: UserTextReferenceOwner) => ReactNode | undefined,
): ReactNode {
  ...
  for (const range of ranges) {
    ...
    const custom = renderReference?.({
      token: text.slice(tokenStart, end), label, displayLabel, kind: referenceKind ?? slashKind,
    })
    if (custom !== undefined) {
      parts.push(<span key={tokenStart}>{custom}</span>)
      cursor = end
      continue
    }
    const contents = <>
