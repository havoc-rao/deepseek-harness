...
The cookie signing secret is the owner-scoped `client-connection/browser-session` grant record in `ctx.credentials`. The local provider persists it in `$DSH_HOME/.credentials.yaml`; `BrowserAuth` loads or creates the record during Connection activation and retains the secret in memory, so request authentication is synchronous. Deleting or replacing the record takes effect on the next Connection activation. Cookies carry an absolute issue/expiry interval, defaulting to 30 days through `cookieMaxAgeDays`. The deterministic cookie name binds the normalized hostname only — a launcher allocates a fresh port per process (Electron picks one per launch), so a port-scoped name would add one durable cookie per launch and the Cookie header would grow past the server's header limit — while the signed payload keeps binding the normalized hostname plus port, so a cookie minted on another port never authorizes this one. They are host-only, `Path=/`, `HttpOnly`, and `SameSite=Strict`; they deliberately omit `Secure` because the shipped server uses loopback HTTP.
