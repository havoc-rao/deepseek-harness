...
export interface ToolRowProps {
   ...
   * value is surviving that clip — the todo row's parallel-active count, a
   * file-mutation row's colored `+A -R` chip. null/absent = the summary is
   * the whole collapsed content (a diff row then falls back to the row's own
   * plain `+A -R` stat; an empty node suppresses it). Dropped on an error
   * row, whose collapsed summary is the failure line instead.
   */
