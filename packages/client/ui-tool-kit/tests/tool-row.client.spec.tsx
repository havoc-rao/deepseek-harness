...
// The row kit's own surface: the pure tool-call row model derivation and the
// ToolRow chrome that every keyed `tool.call.toolview` registrant composes.
// (The ui-tool fallback that drives them through one classified call lives in
// ui-tool's GenericToolCard spec.)
...
} from '../src/client/models/tool-call-model.ts'
...
describe('tool-call-model', () => {
  ...
  it('a search whose queries all filter out falls back to the raw args summary', () => {
    // The join only runs when at least one non-empty string survives the
    // filter; an all-blank/typed list keeps the opaque-args fallback.
    expect(toolRowModel('web_search', running({
      name: 'web_search',
      argsRaw: '{"queries":["", 42]}',
    })).summary).toBe('{"queries":["", 42]}')
  })
  ...
  it('a code row with a non-empty program uses the program as the expanded body', () => {
    const model = toolRowModel('run_code', running({ name: 'run_code', argsRaw: '{"code":"const x = 1"}' }))
    expect(formatToolBody(model.variant, model.bodyRaw ?? '')).toBe('const x = 1')
  })
...
describe('ToolRow', () => {
  ...
  it('Enter and Space on the focused path link stay on the link, not the row', () => {
    const open = vi.fn()
    const view = render(
      <ToolRow {...rowProps} variant="read" title="Read" summary="src/a.ts" filePath="src/a.ts" onOpenFile={open} />,
    )
    const row = view.getByRole('button', { name: /Read/ })
    const link = view.getByRole('button', { name: 'src/a.ts' })
    // The keyboard analogue of the click stopPropagation: the link's keydown
    // handler stops the key reaching the row's toggle, so the row stays
    // collapsed while the link's own activation (native button click) fires.
    fireEvent.keyDown(link, { key: 'Enter' })
    expect(row.getAttribute('aria-expanded')).toBe('false')
    fireEvent.keyDown(link, { key: ' ' })
    expect(row.getAttribute('aria-expanded')).toBe('false')
    // A key neither handler owns bubbles without effect.
