...
The manifest must request the row as a module-table external (the kit is a dynamic row, not a shell-seeded static library, because its models import `abbreviateHomePath`/`resolveWorkspacePath` from `@deepseek-ai/dsh-util-workspace-path`):
...
- **Snapshot-only rows** — each row renders the frozen call/result slice; it does not refetch or reconcile later host state (the running sweep is the one live signal, and it comes from the row's own status stream, not the wire view).
