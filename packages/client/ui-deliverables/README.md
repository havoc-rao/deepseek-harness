...
`deliverablesDefinition` folds each Turn's successful mutation and read calls into engine-published `DeliverablesTurnData`; `producedForClosing` and `readForClosing` read the two sides with the closing Assistant seq. The vocabulary is the tools' own follow-along `locations`, never the closing prose: a produced file is listed whether or not the model remembered to name it. A mutation is recognized by render intent, not tool name — a diff card, or a generic card whose `kind` is `edit` (the shape `str_replace_editor`'s insert presents) — and a read likewise by a generic card whose `kind` is `read` — so a new tool joins by declaring what it does. Deletes and failed calls contribute nothing; a path appears once per Turn in first-seen order on its own side. The Conversation Location index owns Turn membership, so a Turn that acts and then ends without content text cannot spill into the next Turn's row.

`ProducedFiles` renders the row between the closing message's body and its IconActions footer: one measured chip lane per side, **Produced** (the files the turn wrote or edited) above **Read** (the files the turn read), each lane shown only when its side has paths. A lane shows the largest leading prefix that fits (up to six chips; basename text, full path as the `title`) while reserving the exact localized `+ N files` width, so the remainder stays visible without wrapping or horizontal scrolling. Each chip opens through the owner-supplied `openFile` — the same Host opener the tool rows use, with the chat view resolving relative paths against the session cwd. When a lane hides files, its **Show in folder** action opens the session workspace through that same owner path only while the page is loopback and the current Host handshake reports `canOpenPath`; direct remote Web and headless/container Linux Hosts omit the action by default. Design rationale: the [workspace file links Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md).

The row doubles as the talk box's change ledger: below the chip lanes it renders the session-wide totals (`N files · +A -R lines`) folded by the durable `sessionStats` projection from the applied diffs the write/edit tools record in `tool/result` `meta` — distinct files counted once across the whole log, added/removed lines under the diff cards' terminator rule, so the numbers survive paging and compaction and agree with every per-call `+A -R` card. The tail entry claims every turn (not only producing ones), so each talk box records how much the session has changed so far; a turn that only read shows the read lane and the ledger, and a session with no changes (or a composition without the session-stats unit) shows neither. The figures are the tools' applied diffs, not a git stat: only diff-carrying mutation results count, context lines count on both sides, and repeated edits accumulate.
...
- **Change totals follow the tools' applied diffs, not git.** Only mutation results that record applied diffs in `tool/result` `meta` count (write/edit; `str_replace_editor` records none), lines follow the diff-card convention (context lines count on both sides), and repeated edits of one file accumulate instead of netting out.
- **The ledger needs the session-stats unit.** A composition without `@deepseek-ai/dsh-session-stats` serves no `sessionStats` projection value, and the row renders no totals line (produced chips still show).
