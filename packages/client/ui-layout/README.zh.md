...
本包提供 Web GUI 的三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。主题呈现器负责配色、别名 token、正文字号与 document 元数据；右侧栏宽度偏好经 Host 用户设置文档在重启后保留。
...
- **面板几何部分为瞬时状态**——右侧栏宽度偏好与占用方的开合状态经 Host 用户设置文档持久化，但侧栏宽度仍是会话内状态；拖动设置的宽度是整个框架共用的一份偏好，而非每个会话各自的属性。
