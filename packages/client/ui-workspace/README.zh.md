...
Workspace 悬浮卡片会复制对应行被截断的值：激活 Workspace 卡片会写入其完整目录路径，且只有浏览器接受剪贴板写入后，卡片才会显示由字典提供的已复制状态。Session 悬浮卡片完全没有点击复制——点击会把文件列表换成已复制标签，干扰用户阅读。当会话的持久化 `sessionStats` projection 记录了文件领域时，Session 卡片还会分两个区块列出——「输入源」（`read` 工具读过的文件）与「输出源」（写入或编辑过的文件）——最新在前，默认以「文件名 | 路径」的扁平列表直接铺开，工具条上一个低调的按钮可把两侧切换为合并目录树（各最多显示八行，可点击的剩余条数会把该侧展开为完整列表；下层树行附带纵向缩进引导线）；点击任意文件行会以背景色标记该文件便于观察（再点一次清除）；整个文件区内部滚动（限高 + 细滚动条）而非撑高卡片。某侧只有单个文件时以一行扁平路径行展示（VSCode 风格）；多个文件时以目录树展示、每级目录先行，且只含单一子目录的目录链会合并成一行（`src/client/rows/` 显示为一行而非三行）；位于会话工作目录下的路径按相对路径展示（`packages/client/…`，不再带绝对根前缀），之外的路径保持完整形式；某侧没有记录时该侧标题不出现（与所有 projection 消费者的优雅关闭状态一致）。
