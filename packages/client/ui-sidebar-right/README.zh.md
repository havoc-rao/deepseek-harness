...
状态除开合偏好外只在内存中：用户显式的展开与收起选择经 Host 用户设置文档持久化，文档就绪后按这些手势留下的状态重开列——只恢复第一个物化的会话 surface，会话无关停靠面与之后铸造的 surface 都不会恢复，且仅当本次启动尚未发生任何显式手势（放不下面板的窄窗口触发的响应式自动收起从不写入，窄窗口下 seat 的响应式规则仍然优先）。刷新会为第一个物化的会话 surface 恢复显式开合选择（其余 surface 回到折叠默认态）；切换会话则让每个停靠面留在原处。
...
- **几乎全部只在内存中。** 只有用户显式的开合选择持久化（Host 用户设置文档，namespace `ui-sidebar-right`）；其余一切——tab、分栏、浮动、历史——都会在刷新时重置，刷新让每个会话（以及会话无关停靠面）从折叠态开始。
