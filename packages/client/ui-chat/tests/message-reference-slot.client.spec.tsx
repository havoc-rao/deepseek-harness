...
import type { UserTextReferenceOwner } from '@deepseek-ai/dsh-client-ui-primitives'
...
it('routes references through the extension while retaining surrounding text', () => {
  const renderMessageReference = vi.fn((_owner: UserTextReferenceOwner) => <span data-ref-chip="workspace">项目名称</span>)
  render(<UserMessageNodeView {...{ node, t, renderMessageReference, renderMessageImages: () => null } as unknown as ChatNodeViewProps<'user'>} />)
...
it('keeps the official chip when no extension renderer is provided', () => {
  render(<UserMessageNodeView {...{ node, t, renderMessageImages: () => null } as unknown as ChatNodeViewProps<'user'>} />)
  expect(screen.getByText('stable-id').getAttribute('data-ref-chip')).toBe('file')
})

it('declines an extension renderer by returning undefined, falling back to the chip', () => {
  render(<UserMessageNodeView {...{
    node,
    t,
    renderMessageImages: () => null,
    renderMessageReference: () => undefined,
