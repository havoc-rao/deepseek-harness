...
`stop` reads the recorded pid, sends a graceful `SIGTERM` first, escalates to `SIGKILL` after a three-second grace period, and removes the pid file; a stale pid (process already gone) is cleaned up silently. A second `start` while one instance is live fails loud instead of stacking a second window. `restart` pre-validates the desktop app and its binary, then dispatches the stop-then-launch sequence as a detached supervisor relaunching this same CLI (an internal env marker, never the pid file, gates the supervisor body); the command returns instantly, so the old instance's teardown — or the invoking session closing — cannot discard the pending launch (the transcript lands in the log). The supervisor's sequence stays state-independent: a missing or stale pid file is not an error, the stop runs the usual SIGTERM-then-SIGKILL protocol and waits for the old process to exit, and the launch cannot trip `start`'s already-running guard on the instance it just replaced. The app's own main file boots the `web` profile inside a window, so the browser and the desktop surfaces share every requested plugin tree; nothing about that tree lives in the launcher process.
