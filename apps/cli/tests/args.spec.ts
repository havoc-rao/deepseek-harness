...
describe('parseDshArgs', () => {
  ...
  it('routes profile and web config dumps', () => {
    ...
    // Dumps stay boot-free and win over the web launcher actions.
  ...
  it('routes the Electron desktop-app launcher', () => {
    ...
    expect(parse(['electron', 'restart'])).toEqual({ mode: 'electron', action: 'restart', args: [] })
    expect(parse(['electron', 'restart', '--dev', '--some-flag']))
      .toEqual({ mode: 'electron', action: 'restart', args: ['--dev', '--some-flag'] })
