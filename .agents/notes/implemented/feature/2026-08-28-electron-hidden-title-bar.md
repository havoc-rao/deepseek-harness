...
- `ui-conversation` marks the session header's title row (`.header .titleRow` in `ConversationRoot.module.css`) as a drag target so the center column's top chrome drags too; crumb buttons and the `headerActions` / `headerUtilities` / `headerCorner` slot seats stay clickable via `no-drag`.
