...
**The produced-files row is the talk box's ledger.** `ProducedFiles` renders the cumulative totals under the chip lane, reading `useProjection('sessionStats')`; its chain selector claims every turn (not only producing ones), and the component returns null when there is neither a produced chip nor any session change — a read-only turn still records "no new changes" implicitly by showing the running total, and a session with no changes shows nothing. A composition without the session-stats unit serves no projection value and the ledger is simply absent (the chips still show). The web stats strip's window fold is not extended: without the unit, the change fields are absent rather than window-approximated.
