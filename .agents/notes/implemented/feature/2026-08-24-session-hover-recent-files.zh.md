...
`sessionStats` 的 `stateVersion` 现为 4，更早版本持久化的 projection-cache 行会在下次使用时被丢弃并重新折叠。悬停卡片的列表与对话盒合计数字描述的是同一台账的呈现切片，两个表面不可能对"什么算改动"产生分歧。涉及大量文件的会话会让 projection 状态如既往增长；最新 32 个的线上界限使投递的负载保持小。会话悬停卡片把共享的 244px 卡片宽度覆盖为 300px（文件区需要空间容纳路径），行保持完整内容宽度：长路径通过文件框的横向滚动条查看，绝不省略号截断（title 提示仍携带完整路径）。
