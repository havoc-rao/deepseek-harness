...
- **Per-hunk +/− badge.** Each hunk header — the path row, or a same-file second hunk's `⋯` gap — carries a small `+A -R` pill with the nonzero terms only, so a reader sees one hunk's move size next to the path, distinct from the footer's whole-card totals. The counts come from the rows `buildRows` already derives, so the badge adds no wire field or backend contract change.
