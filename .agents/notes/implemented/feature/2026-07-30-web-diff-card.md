...
- **Per-hunk +/− badge.** Each hunk header — the path row, or a same-file second hunk's `⋯` gap — carries a small `+A -R` pill with the nonzero terms only, so a reader sees one hunk's move size next to the path, distinct from the footer's whole-card totals. The counts come from the rows `buildRows` already derives, so the badge adds no wire field or backend contract change. The collapsed row mirrors the totals at its right edge: `FileMutationRow` derives the call's total `+A -R` (nonzero terms only) through the shared `diffLineCounts` helper and passes it as ToolRow's `summarySuffix`, so the change magnitude reads without expanding the card; `diffLineCounts` lives in `ui-primitives` so the row and the card share one line-terminator rule. The suffix is colored to match the in-card badge — `+` on the success token, `-` on the error token — so the row's trailing fragment reads the same as the card: `summarySuffix` accepts a `ReactNode` (a plain string still works, as the todo row's `+n` count uses), and `FileMutationRow`'s `file-mutation-row.module.css` owns the two color spans while ToolRow's `.summarySuffix` slot keeps the geometry. The terms sit in a small layer-1 chip (the same pill surface the in-card hunk badge uses) so the totals read as a badge, not bare trailing text.
...
`packages/client/ui-primitives/tests/diff-block.client.spec.tsx` pins the component: the create arm (added-only, no removed side), the edit arm (removed above added), the same-file `⋯` gap versus a new file's own header, the per-hunk badge (both terms on an edit, the added term only on a create, the removed term only on a full deletion, a same-file second hunk's badge on its `⋯` gap, unbadged change lines, and a no-op hunk omitting the badge), the empty-diffs null render, the footer counts and their singular/plural, the head/tail cap with its `aria-expanded` toggle, and the copy control asserting the prefixed diff text on both the accepted and refused clipboard paths. Per-file 100%.

`packages/client/ui-tool-kit/tests/diff-card-model.client.spec.tsx` pins `diffCardModel`'s derivation and each of its null arms; `packages/client/ui-toolview-file-mutation/tests/file-mutation-row.client.spec.tsx` pins the keyed row — its resident card, the path link opening cwd-resolved through the host, the trailing `+A -R` suffix on the collapsed summary (settled, running, create, no-op, and errored arms), its registration under both `write` and `edit`; the ui-tool suite pins the fallback chat row body and the panel's Output section. `diffLineCounts` is pinned in the primitive spec alongside the card.
