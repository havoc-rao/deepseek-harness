...
- **`dsh-hooks-claude-code`** — the CC dialect. Nine of Claude Code's current hook points: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStart`, `SubagentStop`, `Notification`, and `PermissionRequest`. Owns CC-shaped per-event stdin payloads (a base of `session_id`/`transcript_path`/`cwd`/`hook_event_name` plus per-event fields), `CLAUDE_PROJECT_DIR` plus `${CLAUDE_PLUGIN_ROOT}`/`${CLAUDE_PROJECT_DIR}` substitution, and the literal-or-regex matcher mode. `transcript_path` is the persistence locator result or `''`; stdin carries a **trailing newline**.
...
`Notification` and `PermissionRequest` are the same detached, contained shape: they fire on the `approval/request` waterfall before any answering listener (`prepend`), and `Notification` also fires on the `userQuestions/ask` emit that the `dsh-user-questions` seam now raises the instant a question reaches its UI provider. The approval observer delegates (it always calls `next()`) and both runs detach, so a slow or failing hook can neither delay nor fail the approval/question it announces — and `runHook` never rejects, so the fire cannot throw into the waterfall. `PermissionRequest` carries `permission_mode: "ask"` plus the tool identity/`reason` for the approval under decision; it observes only and cannot itself grant (the composed answerers hold the decision). Both write no `hook/*` pair (emit-shaped points like SessionStart); the waits they announce stay audited by the seam's own `approval/asked`/`approval/decided` records and the tool result.
