...
The host half registers the [`ui-theme.preference` settings section](2026-08-06-host-backed-web-preferences.md) (and the `ui-theme.paper` tone field added by the [paper-tone axis](../feature/2026-08-28-web-paper-tone-axis.md)) when a settings provider exists. For each index response, it embeds that schema-validated built-in preference and paper tone in the inline script; without a settings provider or active registration, it embeds the `system` preference and `default` tone. The browser resolves `system` through `prefers-color-scheme`, falling back to light when `matchMedia` is unavailable. It writes only the DOM state that ThemePresenter later owns: `document.documentElement.style.colorScheme`, `body[data-ds-dark-theme]`, and the tone's per-scheme alias-token variants as inline body variables so the loading page's first paint is already tinted.

The bootstrap logic recognizes only the built-in `light`, `dark`, and `system` semantics plus the built-in paper-tone table. It registers no listeners and does not resolve third-party themes or token overrides. After the browser-side plugin tree activates, ThemeRuntime remains authoritative for theme state, and ThemePresenter writes the complete resolved result back to the same DOM state and owns subsequent updates and disposal.
...
The loading page's first frame matches the durable built-in preference and paper tone, and defaults to the OS preference when no settings provider is composed. The index transform reads Host settings for every response, while the inline script contains only the selected built-in values and `system` resolution. Changes to the built-in preference or tone semantics, or to ThemePresenter DOM fields, must update both the script and ThemeRuntime. A custom theme still applies fully only after the browser plugins activate; during the loading interval, the page uses its private light or dark fallback palette.
