...
The host half registers the [`ui-theme.preference` settings section](2026-08-06-host-backed-web-preferences.md) (the `ui-theme.paper` tone field added by the [paper-tone axis](../feature/2026-08-28-web-paper-tone-axis.md) rides the same schema) when a settings provider exists. For each index response, it embeds that schema-validated built-in preference in the inline script; without a settings provider or active registration, it embeds the `system` default. The browser resolves `system` through `prefers-color-scheme`, falling back to light when `matchMedia` is unavailable. It writes only the DOM state that ThemePresenter later owns: `document.documentElement.style.colorScheme` and `body[data-ds-dark-theme]`. The ui-paper plugin contributes its own body row for the paper-tone axis: it embeds the durable tone and preference from the same section and writes the tone's per-scheme alias-token variants as inline body variables so the loading page's first paint is also tinted.
...
The loading page's first frame matches the durable built-in preference and (through the ui-paper row) the paper tone, and defaults to the OS preference when no settings provider is composed. The index transform reads Host settings for every response, while the inline scripts contain only the selected built-in values and `system` resolution. Changes to the built-in preference or tone semantics, or to ThemePresenter DOM fields, must update the scripts and ThemeRuntime together. A custom theme still applies fully only after the browser plugins activate; during the loading interval, the page uses its private light or dark fallback palette.
